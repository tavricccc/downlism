...
/// </summary>
/// <remarks>
/// Downlism's WinUI is framework-dependent: the Windows App SDK binaries come from one MSIX
/// framework package that Windows keeps in a single shared location, instead of a 145 MB copy
/// inside every installation. Flowlism and Peeklism point at the same package, so the three of
...
{
    /// <summary>
    /// The framework package Downlism's WinUI resolves at run time. The family name is fixed by
