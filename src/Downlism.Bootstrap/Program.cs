...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            root = Path.Combine(Path.GetTempPath(), "Downlism.Setup", Guid.NewGuid().ToString("N"));
            ...
            var start = new ProcessStartInfo(Path.Combine(work, "Downlism.Setup.exe"))
            {
        ...
        {
            MessageBox(0, "無法開啟安裝程式。\n" + exception.Message, "Downlism", IconError);
            return 1;
        }
        finally
        {
    ...
    {
        // Already registered, by an earlier install or by Flowlism or Peeklism: nothing to ask
        ...
        var answer = MessageBox(
            0,
            "Downlism 需要 Microsoft 的 Windows App 執行環境，這台電腦尚未完整安裝。\n\n"
                + "要登錄缺少的共用元件嗎？元件已經在這個安裝程式裡，不需要連網，"
                + "登錄後由 Windows 集中保管一份，之後 Flowlism、Peeklism 也共用它，"
                + "省下約 145 MB 的磁碟空間。\n\n"
                + "選「否」也可以照常安裝：Downlism 會自己帶一份執行環境，不登錄任何共用元件。",
            "Downlism",
            IconQuestion);
        ...
        try
        {
            using var progress = new ProgressDialog("Downlism", "正在登錄 Windows App 執行環境…");
    ...
    {
        using var progress = new ProgressDialog("Downlism", "正在準備安裝內容…");
