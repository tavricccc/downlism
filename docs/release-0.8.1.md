...
196 項 C# 測試通過，涵蓋時間加權平均、暫停續傳、統計保存、舊資料庫遷移、工具流量排除，以及安裝／移除的實際進度。App 與安裝程式建置均無警告或錯誤。
...
安裝檔：`artifacts/installer/Downlism.Setup.exe`。建置紀錄：`artifacts/publish-0.8.1.log`。
