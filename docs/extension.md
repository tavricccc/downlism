...
**右鍵選單。** 「用 Downlism 下載」對連結、圖片、影片、音訊都能用，適合點擊攔截刻意放過的那些。另外在頁面本身上有「用 Downlism 下載這個頁面的影片」——現代網站的 `<video>` 幾乎沒有可用的 `src`，只有頁面網址交給 yt-dlp 才有意義。

磁力連結只在右鍵選單與點擊攔截兩條路上被接受，其餘位置一律只收 http／https。理由正好相反於 `file://` 的理由：磁力連結只可能指向一個 swarm，永遠不可能指向這台機器上的一個路徑。

## 影片嗅探

前四條路都在處理「瀏覽器打算存成檔案」的東西。頁面裡播放的影片不是其中之一——它不會出現在 `chrome.downloads` 裡，任何一條路都看不到。

嗅探是第五條路：`webRequest.onHeadersReceived` 上另外掛一個監聽器，對 `media`、`xmlhttprequest`、`object`、`other` 分類回應。規則全部是為了「丟掉東西」而寫的：

- manifest（`.m3u8`、`.mpd`、`.f4m`、`.ism`，或對應的 content-type）一律收下，一個 manifest 描述整支影片
- `video/*`、`audio/*` 或完整媒體副檔名、且大於 512 KB 的收下
- `.ts`、`.m4s`、`.aac`、`.vtt` 這類片段一律丟掉

最後一條是能不能用的關鍵。一小時的影片是數千個片段，把它們列出來，唯一有用的那一筆 manifest 會被埋在最底下。

YouTube、bilibili、X 這類網站直接不嗅探：它們的媒體網址是簽章過、會過期、分段取用的，單獨拿出來不會下載成功。彈出視窗對這些網站改為只提供「整頁交給 yt-dlp 下載」。

嗅探結果存在 `chrome.storage.session` 並以分頁為單位，導覽或關閉分頁時清掉。不放在模組變數裡，是因為服務工作者閒置三十秒就會被回收，而一個頁面可以播上一小時。圖示上的數字徽章也是分頁專屬的，屬於使用者正在看的那一頁。

判斷規則在 `src/media.ts`，與瀏覽器 API 無關，由 `npm test` 覆蓋。
...
彈出視窗現在有三段：這個頁面上嗅探到的影片（每一筆按一下就交給 Downlism）、「整頁交給 yt-dlp 下載」，以及原本的兩項設定。

串流交給影片引擎，完整檔案則走一般的分段下載——一個能直接抓下來的 mp4 沒有理由多開一個 yt-dlp 行程。
